...
import logging
import hydra
from utils_hydra import load_data, return_label, BagProcessor, safe_bootstrap
...
from loggers.db import LogDataBase
import json
import pickle
import re
import pprint
...
def log_stats(cfg):
    ''' 
    Output relevant stats at the beggining of the run.'''
...
@hydra.main(version_base=None, config_path="configs", config_name="probe_mil")
def main(cfg: DictConfig):
    ...
    task = Task(cfg.task)
    # DataBase to log the progress locally
    db = LogDataBase(
    ...
    for layer_id in layers:
        ...
        _y_train, r_train, _, _, _, _ = return_label(data_train)

        # LOAD THE TEST DATA
        data_test = dh_test.get_test_df().reset_index(drop=True)
        _y_test, r_test, _, _, _, _ = return_label(data_test)

        # LOAD THE CALIBRATION DATA
        if dh.with_calibration:
            data_cal = dh.get_cal_df().reset_index(drop=True)
            _y_cal, r_cal, _, _, _, _ = return_label(data_cal)
        ...
        if cfg.search:
            _ = runner.parameter_search(
                X=X_tr, y=y_train, mask=mask)
        else:
            try:
                _ = runner.single_training(
                    X=X_tr, y=y_train, mask=mask)
            except Exception as e:
                log.error(f"Error: {e}")
                log.warning(
