...
from response.prompt_templates import (
    MultichoicePrompt,
    MultichoicePromptABC,
    MultichoicePromptTF,
)
from utils_hydra import get_device, load_statements_with_targets, prepare_hf_model
...
def validate_config(cfg: DictConfig):
    assert (
        type(cfg.datasets) == list or type(cfg.datasets).__name__ == "ListConfig"
    ), f"Datasets paramaeter must be a list. Not {type(cfg.datasets)}"
...
def main(cfg: DictConfig):
    ...
    for dataset in cfg.datasets:
        ...
        if cfg.limit_batches > 0:
            batches = np.array_split(statetements, n_batches)[: cfg.limit_batches]
        else:
        ...
        if cfg.model["instruct"]:
            ...
            _prompt = re.sub(
                rf"{re.escape(end_token)}\s*$",
                " ",
                tokenizer.apply_chat_template(
                    prompt_f(batches[0][0]),
                    tokenize=False,
        ...
        log.warning(f"Examples of a prompt:\n---\n {_prompt}\n---")
        ...
        for i, batch in enumerate(tqdm(batches, total=len(batches))):
            ...
            input_ids, input_att = tokenized["input_ids"].to(cfg.device), tokenized[
                "attention_mask"
            ].to(cfg.device)
            if i in __log_intervals:
                status = i / n_batches
                log.warning(f"(BATCH) Processed {status:.2%} of the statements")
            if cfg.question_type == "binary":
                raise NotImplementedError()
            elif cfg.question_type == "multichoice":
                out = model(input_ids, attention_mask=input_att, use_cache=False).logits
        ...
        if cfg.limit_batches < 0:
            assert scores.shape[0] == len(
                statetements
            ), "Scores and statements must have the same length"
        ...
        log.warning("(BATCH) Processed 100.00% of the statements\n\n")
...
def instruct_tokenize(batch, tokenizer, cfg):
    batch = tokenizer.apply_chat_template(
        batch, tokenize=False, add_generation_prompt=False, continue_final_message=True
    )
    if cfg.model["end_token"] == "auto":
        end_token = tokenizer.eos_token
    else:
        end_token = cfg.model["end_token"]
    text_batch = [b.strip(" ").rstrip(end_token).strip(" ") + " " for b in batch]
