...
class TTPD(ClassifierMixin, BaseEstimator):
    ...
    def _get_truth_direction(X: np.ndarray, t: np.ndarray, p: np.ndarray) -> np.ndarray:
        ...
        assert np.unique(p).tolist() == [-1, 1] or np.unique(p).tolist() == [
            0
        ], "p must be in {-1, 1} or {0}"
        ...
        if np.all(p == 0):
            ...
            A = np.column_stack([t_copy, t_copy * p_copy])  # (N,2)
    ...
    @staticmethod
    def _get_polarity_direction(
        X: np.ndarray, p: np.ndarray, seed: int = 42
    ) -> np.ndarray:
        """
        ...
        """
        assert np.unique(p).tolist() == [0, 1] or np.unique(p).tolist() == [
            0
        ], "p must be in {0, 1} or {0}"
    ...
    def fit(self, X: np.array, t_labels: np.array, p_labels: np.array) -> TTPD:
        ...
        """
        assert (
            X.shape[0] == t_labels.shape[0] == p_labels.shape[0]
        ...
        self.w_t, self.w_p_ = self._get_truth_direction(
            X=X, t=self._labels_to_sign(t_labels), p=self._labels_to_sign(p_labels)
        )
