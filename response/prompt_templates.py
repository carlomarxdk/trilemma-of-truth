...
class PromptTemplate(ABC):
    ...
    def __init__(
        ...
        enumeration: list = None,
        system_role: str = "system",
        user_role: str = "user",
        assist_role: str = "assistant",
    ):
    ...
    def templates(self) -> dict[str, Callable[[str], str | list[dict]]]:
        return {
            "default": lambda x: f"Question: Is the following statement correct? {x}\n\n"
            "Select one of the following options:\n"
            f"{self.options_as_str()}\n\n"
            f"Please respond with the corresponding {self.enum_unit}. "
            f"The final answer is {self.enum_unit} ",
            "masked": lambda x: f"Question: Is the following statement correct? {x}\n\n"
            "Select one of the following options:\n"
            f"{self.options_as_str()}\n\n"
            f"Please respond with the corresponding {self.enum_unit}. "
    ...
    def task(self, task: int) -> None:
        assert task in [0, 1], "Task must be 0 for False -> Truth or 1 for Null->Know"
        self._task = task
