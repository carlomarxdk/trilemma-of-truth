...
@pytest.mark.parametrize("RunnerClass,bagged,task, probe", [
...
def test_runner_predictions_have_output_and_consistency(RunnerClass, bagged, task, probe):
    """Test that each runner produces output and (optionally) consistent predict/inst_predict results."""
    MODEL_NAME = 'llama-3-8b'
    DATASET_NAME = 'cities_loc'

    output_dir = Path('outputs') / 'probes' / probe / \
        MODEL_NAME / f'{ DATASET_NAME }_search_task-{str(task)}'
    ...
    runner = RunnerClass(cfg=cfg).load(
        output_dir=output_dir, layer_id=layer_id)
    ...
    assert bag_preds is not None and len(
        bag_preds) > 0, f"{RunnerClass.__name__}: bag_predict returned empty"
    assert preds is not None and len(
        preds) > 0, f"{RunnerClass.__name__}: predict returned empty"
    assert inst_preds is not None and len(
        inst_preds) > 0, f"{RunnerClass.__name__}: inst_predict returned empty"
    ...
    if bagged:
        ...
        np.testing.assert_allclose(
            preds, inst_preds, err_msg=f"{RunnerClass.__name__}: predict != inst_predict")
