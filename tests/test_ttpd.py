...
def make_synthetic(n=400, d=16, rng=0):
    ...
    w_t_true = rs.randn(d)
    w_t_true /= np.linalg.norm(w_t_true)
    w_p_true = rs.randn(d)
    w_p_true /= np.linalg.norm(w_p_true)
...
def test_get_truth_direction_zero_polarity_returns_single_direction():
    ...
    cos = np.dot(w_t_hat, w_t_true) / (
        np.linalg.norm(w_t_hat) * np.linalg.norm(w_t_true)
    )
    assert cos > 0.9
...
def test_fit_predict_end_to_end_high_auc():
    ...
    model = TTPD(
        base=LogisticRegression(penalty=None, fit_intercept=True, max_iter=2000),
        random_seed=123,
    )
    model.fit(X, y, p)
