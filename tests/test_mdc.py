...
def make_gaussian_binary(n=400, d=32, sep=1.0, cov_scale=1.0, seed=0, equal_cov=True):
    ...
    mu1 = np.zeros(d)
    mu1[0] = sep
    A0 = g.standard_normal((d, d))
    S0 = (A0 @ A0.T) / d
    A1 = g.standard_normal((d, d))
    S1 = (A1 @ A1.T) / d
    if equal_cov:
        S1 = S0
...
def test_fit_with_covariance_matches_lda_when_equal_cov():
    ...
    X, y, _, _, _, _ = make_gaussian_binary(
        n=600, d=10, sep=1.2, seed=42, equal_cov=True
    )
    # Our classifier (Mahalanobis)
    mdc = MeanDifferenceClassifier(
        with_covariance=True, cov_reg=1e-8, fit_intercept=True, cov_type="empirical"
    ).fit(X, y)
    # sklearn LDA
...
def test_external_mahalanobis_matrix_path():
    ...
    M = np.diag(1.0 / var)
    clf = MeanDifferenceClassifier(with_covariance=True, fit_intercept=True).fit(
        X, y, M=M
    )
...
def test_handles_small_n_big_d_with_shrinkage():
    ...
    X, y, _, _, _, _ = make_gaussian_binary(
        n=50, d=128, sep=2.0, seed=7, equal_cov=True
    )
