...
log = logging.getLogger("MulticlassSILRunner")

#### Run multiclass SIL-SVM etc. 

class MulticlassSILRunner:
    """
    ...
    def __init__(self, cfg):
        ...
        # hyperparams
        self.pool = "last" # pools only last token
        self.max_iter = 2000
    ...
    def single_training(self, X, y, mask, layer_id=None, **kwargs):
        ...
        """
        assert self.pool == "last", "Only inference/training based on the last token is supported"
    ...
    def predict_proba(self, X, is_test = False):
        if is_test:
            self.separator.set_pool("max_sil")
        else:
            self.separator.set_pool("last")
        return self.separator.predict_proba(X)  # (N, C)

    def predict(self, X, is_test=False):
        if is_test:
            self.separator.set_pool("max")
        else:
            self.separator.set_pool("last")
